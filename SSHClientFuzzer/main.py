...
def main():
    while 1:
        server = SSHserver.SSHserver(BIND_IP = bind_ip, fuzz_style=FUZZ_STYLE.SNIPER, fuzz_severity=0.9)
      #  proc = subprocess.Popen(ssh_client, stdout=subprocess.PIPE, shell=True)
        
        server.Run()
    #    subprocess.call('/mnt/c/Windows/System32/taskkill.exe /F /IM BvSsh.exe', stdout=subprocess.PIPE, shell=True)
        server.Stop()
